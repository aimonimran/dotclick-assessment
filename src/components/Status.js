...
function Status({ status }) {
	return status === 'in-process' ? (
		<div className='status justify-between text-gray-400 uppercase text-xs md:text-sm font-semibold'>
		...
		<div className='status'>
			<p className='w-full py-2 pl-4 bg-[#44A16F] text-white uppercase text-xs md:text-sm font-semibold'>
				Completed
			</p>
		</div>
	);
